            ...
            <table id="customerTable" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    @foreach ($customers as $customer)
                        <tr>
                            <td>
                                <a href="{{ route('customer.edit', $customer->id) }}" class="btn btn-warning btn-custom">Editar</a>
...
@section('js')
    ...
    <script>
        function modal(id, action) {
            const actionText = action === 'delete' ? 'desactivar' : 'restaurar';
            $('#nombre').html(`¿Seguro que deseas ${actionText} el cliente con ID: ${id}?`);
            const url = action === 'delete'
                ? "{{ route('customer.destroy', ':id') }}".replace(':id', id)
                : "{{ route('customer.restore', ':id') }}".replace(':id', id);
            $('#action-form').attr('action', url);
            $('#method-input').val(action === 'delete' ? 'DELETE' : 'PUT');
            $('#confirm-button')
                .toggleClass('btn-danger', action === 'delete')
                .toggleClass('btn-success', action === 'restore')
